...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.articleSet.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.article-sets.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('articles') ? 'is-invalid' : '' }}" name="articles[]" id="articles" multiple>
                    @foreach($articles as $id => $article)
                        <option value="{{ $id }}" {{ in_array($id, old('articles', [])) ? 'selected' : '' }}>{{ $article }}</option>
                    @endforeach
                </select>
